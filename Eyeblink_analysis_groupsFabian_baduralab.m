...
Folder = 'Z:\Maria\Eyeblink Behavior Data\males'; 
...
for k = 1 : length(Files)
    ...
    amplitudeCR_CSUS_sessions_std=grpstats(amplitudesCR(ttype>1),sesno(ttype>1),{'std'});
    ...
    amplitudeCR_CS_sessions_std=grpstats(amplitudesCR(ttype==3),sesno(ttype==3),{'std'});
    ...
    save([num2str(mouseno),'summary'],'tracesnorm','traces_US_trials','traces_CSUS_trials', 'traces_CS_trials', 'ttype','mouseno','sesno'...
       ...
       ,'amplitudeCR_CSUS_sessions_std','amplitudeCR_CS_sessions_std');
...
%      sum_amplitudesCR = cat(3,sum_amplitudesCR, amplitudesCR); 
...
average_amplitudesCR = mean (sum_amplitudesCR,3); 
std_amplitudesCR = std (sum_amplitudesCR,[],3);
sem_amplitudesCR = std_amplitudesCR/sqrt(length(average_amplitudesCR)); 


save(('Results_groupX'),'average_amplitudeCR_CS_sessions','std_amplitudeCR_CS_sessions','std_amplitudesCR','std_amplitudeCR_CSUS_sessions', 'average_amplitudeCR_CSUS_sessions', 'average_amplitudesCR'...
    ,'sem_amplitudesCR','sem_amplitudeCR_CSUS_sessions','sem_amplitudeCR_CS_sessions','average_traces_CS_trials', 'average_traces_CSUS_trials', 'average_traces_US_trials');
...
%% Plot CR amplitude per session - mean and std 

figure
plot(sessions,amplitudeCR_CSUS_sessions); 
errorbar(amplitudeCR_CSUS_sessions,std_amplitudeCR_CSUS_sessions); 
...
figure
plot(sessions,amplitudeCR_CS_sessions); 
errorbar(amplitudeCR_CS_sessions,std_amplitudeCR_CS_sessions); 
...
%% Plot CR amplitude per session - mean and sem 

figure
plot(sessions,amplitudeCR_CSUS_sessions); 
errorbar(amplitudeCR_CSUS_sessions,sem_amplitudeCR_CSUS_sessions); 
...
figure
plot(sessions,amplitudeCR_CS_sessions); 
errorbar(amplitudeCR_CS_sessions,sem_amplitudeCR_CS_sessions); 
...
%% Plot CR amplitude per session. - boxplots
